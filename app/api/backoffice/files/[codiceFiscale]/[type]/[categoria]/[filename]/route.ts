...
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ codiceFiscale: string; type: string; categoria: string; filename: string }> }
) {
  try {
    ...
    const { codiceFiscale, type, categoria, filename } = params;
    ...
    if (!fs.existsSync(filePath)) {
      ...
      return NextResponse.json(
        { error: 'File non trovato', path: filePath },
        { status: 404 }
      );
    }
